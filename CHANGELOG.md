...
- Validate core Medchem on Linux, Windows, macOS Apple Silicon and macOS Intel;
  ...
  separate jobs.
- Keep publication manual through the `release` action and the `PYPI_API_TOKEN`
  secret. Release tests and isolated wheel/source checks gate publication;
  prereleases never replace the stable documentation.
