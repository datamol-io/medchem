...
1. Finalize the release notes in `CHANGELOG.md` with an exact version heading,
   for example `## 3.0.0 - YYYY-MM-DD`. Date the release and update README
...
`3.0.0` in **release-version**, and leave **dry-run** checked for a
...
unchecked. Use canonical versions without a `v` prefix, such as `3.0.0`
or `3.0.0rc1`; `a`, `b` and `rc` suffixes identify prereleases.
