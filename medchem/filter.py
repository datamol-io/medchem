...
def score(smiles_list,
    ...
    if os.path.isfile(os.path.join(CHARGE_ASSIGN_DIR, "queries")):
        extra_iwdemerit_options += " -N F:" + os.path.join(
            CHARGE_ASSIGN_DIR, "queries")
    ...
    if odm:
        ...
        with open(query_files[2]) as QRY_IN:
            ...
            omit_queries = [
                qry for odm_p in odm_patterns for qry in set(current_queries)
    ...
    smiles_file = None
    with tempfile.NamedTemporaryFile(mode="w+t", suffix=".smi",
                                     delete=False) as smiles_tmp_files:
        smiles_tmp_files.write("\n".join(
            [f"{sm}\t{i}" for i, sm in enumerate(smiles_list)]))
    ...
    if stop_after_step >= 1:
        cmd = []
        cmd.extend((BIN2PATH["tsubstructure"] +
                    " -E autocreate -b -u -i smi -o smi -A D ").split())
        cmd.extend(("-m " + bad_file_1 + " -m QDT").split())
        cmd.extend((f"-n {tsub_out_1} -q F:" + query_files[0] +
                    optional_queries + f" {mc_pass_out}").split())
        out.append(run_cmd(cmd))
    if stop_after_step >= 2:
        cmd = []
        cmd.extend((BIN2PATH["tsubstructure"] +
                    " -A D -E autocreate -b -u -i smi -o smi ").split())
        cmd.extend(("-m " + bad_file_2 + " -m QDT").split())
        cmd.extend((f"-n {tsub_out_2} -q F:" + query_files[1] +
                    f" {tsub_out_1}").split())
        out.append(run_cmd(cmd))

    if stop_after_step >= 3:
        cmd = []
        cmd.extend((BIN2PATH["iwdemerit"] + " -u -k -x -t " +
                    extra_iwdemerit_options +
                    " -E autocreate -A D -i smi -o smi -q F:" +
                    query_files[2]).split())
        cmd.extend(f"-R {bad_file_3}".split())
        cmd.extend(
            f"-G {iwd_out} -c smax={soft_upper_atom} -c hmax={hard_upper_atom} {tsub_out_2}"
            .split())
        out.append(run_cmd(cmd))
