...
#     define bzero(s, n)  (memset (s, '\0', n), (s))
#undef memcpy
#     define memcpy(d, s, n)    (strncpy(d, s, n))
#undef memcmp
#endif
...
int
regcompn (preg, pattern, lenpattern, cflags)
regex_t *preg;
const char *pattern;
size_t lenpattern;
int cflags;
{
  ...
  ret = regex_compile (pattern, lenpattern, syntax, preg);
...
int cflags;
{
  return regcompn (preg, pattern, strlen (pattern), cflags);
}
...
int
regexecn (preg, string, len, nmatch, pmatch, eflags)
const regex_t *preg;
const char *string;
size_t len;
size_t nmatch;
...
size_t
regerror (errcodeX, preg, errbuf, errbuf_size)
int errcodeX;
const regex_t *preg;
char *errbuf;
size_t errbuf_size;
{
  ...
  if (errcodeX < 0
      || errcodeX >= (int) (sizeof (re_error_msgid_idx)
  ...
  msg = gettext (re_error_msgid + re_error_msgid_idx[errcodeX]);
